...
// tr? v? t?a d? x hi?n t?i c?a con tr? chu?t
int whereX()
{
...
// tr? v? t?a d? y hi?n t?i c?a con tr? chu?t
int whereY()
{
	CONSOLE_SCREEN_BUFFER_INFO csbi;
	if (GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &csbi))
...
// Hi?n th? các giá tr? ra màn h?nh
...
// ki?m tra c?n v? trí tr?ng hay không
bool checkZero()
{
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
